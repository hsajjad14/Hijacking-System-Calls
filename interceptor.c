...
mytable table[NR_syscalls]; // NR_syscalls = number of system calls
...
asmlinkage long interceptor(struct pt_regs reg) {
	long retval;
	int sys_call_num = reg.ax; // get syscal num by reg.ax

    spin_lock(&my_table_lock);

    // check pid is monitored
	if((check_pid_monitored(sys_call_num, current->pid)) == 1 || table[sys_call_num].monitored == 2){
		log_message(current->pid, (long unsigned int)sys_call_num, (long unsigned int)reg.bx, (long unsigned int)reg.cx, (long unsigned int)reg.dx, (long unsigned int)reg.si, (long unsigned int)reg.di, (long unsigned int)reg.bp);
	}

    retval = table[sys_call_num].f(reg);
    spin_unlock(&my_table_lock);
	return retval;
}

asmlinkage long handle_request_syscall_intercept(int syscall){
	// check user is root
	if (current_uid() == 0){
		spin_lock(&my_table_lock);
...
asmlinkage long handle_request_syscall_release(int syscall){
	// must be root
	if (current_uid() == 0){
...
asmlinkage long handle_request_start_monitoring_one(int syscall, int pid){
	// must be root
	if (current_uid() == 0 || check_pids_same_owner(current->pid, pid) == 0){
		...
		if((table[syscall].monitored == 1 && check_pid_monitored(syscall, pid) == 0) ||
			(table[syscall].monitored == 0)){

			// pid should be monitored by syscall
...
asmlinkage long handle_request_start_monitoring_all(int syscall){

	if (current_uid() == 0){
		spin_lock(&my_table_lock);
		// add every pid to be monitored by the syscall
		if (table[syscall].monitored == 2){
			spin_unlock(&my_table_lock);
			return -EBUSY;
		}
...
asmlinkage long handle_request_start_monitoring(int syscall, int pid){
	// check if pid is valid (exists)
	if (pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){
		return handle_request_start_monitoring_one(syscall, pid);
	}else if(pid == 0 ){ // start monitoring all pids
		// check user is root
		return handle_request_start_monitoring_all(syscall);
	}else{
		return -EINVAL;
	}
	return 0;
...
asmlinkage long handle_request_stop_monitoring_one(int syscall, int pid){
	if (current_uid() == 0 || check_pids_same_owner(current->pid, pid) == 0){
		spin_lock(&my_table_lock);
		if (table[syscall].monitored == 2){
			spin_unlock(&my_table_lock);
...
asmlinkage long handle_request_stop_monitoring_all(int syscall){
	if (current_uid() == 0){
		spin_lock(&my_table_lock);
		if (table[syscall].monitored == 0){
			spin_unlock(&my_table_lock);
...
asmlinkage long handle_request_stop_monitoring(int syscall, int pid){
	// check if pid is valid (exists)
	if (pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){
		return handle_request_stop_monitoring_one(syscall, pid);
	} else if(pid == 0){ // stop monitoring all pids
		// check user is root
		return handle_request_stop_monitoring_all(syscall);
	}else{
		return -EINVAL;
	}
return 0;
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	if (syscall > 0  && syscall <= NR_syscalls - 1){
		if (cmd == REQUEST_SYSCALL_INTERCEPT){
			return handle_request_syscall_intercept(syscall);
		}else if(cmd == REQUEST_SYSCALL_RELEASE){
			return handle_request_syscall_release(syscall);
		}else if(cmd == REQUEST_START_MONITORING){
			return handle_request_start_monitoring(syscall, pid);
		}else if(cmd == REQUEST_STOP_MONITORING){
			return handle_request_stop_monitoring(syscall, pid);
		}else{
			//invalid command
			return -EINVAL;
		}
 ...
 */
 static int init_function(void) {
    ...
    spin_lock(&sys_call_table_lock);
 	// Make system_call_table R/W before changing it
 	set_addr_rw((unsigned long)sys_call_table);
